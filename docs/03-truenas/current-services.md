...
### Pi-hole (DNS)
- **Type:** TrueNAS App
- **Status:** Now actually deployed and running (previously planned for a Raspberry Pi instead; that plan has not happened, and Pi-hole runs on TrueNAS today)
- **Purpose:** Network-wide ad blocking and DNS
- **Migration Priority:** Reassess. The original plan was to run this on a Pi for redundancy independent of TrueNAS uptime; that tradeoff is now unaddressed
...
`11434` returns nothing and makes a perfectly healthy Ollama look stopped. It is a
...
Other subdomains referenced in earlier docs (jellyseerr, etc.) were not re-verified this pass; confirm the NPM proxy host list directly before relying on this.
