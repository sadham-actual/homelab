...
This was not theoretical risk. The Actual Budget LXC had already lost its `account.sqlite` to a zero-byte corruption once (2026-07-24) and was recovered only by a password reset, with the wreckage still on disk. A second such event, or any node-local storage failure, would have been unrecoverable, since all guest disks live on node-local LVM-thin with no redundancy (see [Proxmox Cluster Hardware](../docs/02-hardware/proxmox-node.md)).
...
- Roughly 14 restore points for ~10GB against multiple TB free, so retention is effectively free at this scale.
...
- Backups depend on TrueNAS being up and the NFS export being present. If the share disappears, the job fails, and silently, per the previous point.
