...
Dell specifies both 65W and 90W adapters as valid for this OptiPlex 3000 Micro configuration (35W CPU), meaning the 65W PD board this node was using was at the edge of spec, not comfortably above it, before accounting for extra conversion losses in the PD-board/adapter chain. Bringing multiple CPU cores online during ACPI/SMP initialization causes a brief current spike well above steady-state draw, which is exactly the moment the crash occurred.
...
**Positive:**
- Full CPU capacity (12 threads) restored on this node, which can now host real VM/LXC workloads instead of sitting idle.
